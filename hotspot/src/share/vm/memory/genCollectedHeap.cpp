...
GenCollectedHeap::GenCollectedHeap(GenCollectorPolicy *policy) :
		SharedHeap(policy), _gen_policy(policy), _gen_process_strong_tasks(
				new SubTasksDone(GCH_PS_NumElements)), _full_collections_completed(
				0) {
...
jint GenCollectedHeap::initialize() {
	...
	if (UseSharedSpaces) {
		if (!heap_rs.is_reserved() || heap_address != heap_rs.base()) {
			...
			allocate(alignment, perm_gen_spec, &total_reserved,
					&n_covered_regions, &heap_rs);
		}
	}
...
char* GenCollectedHeap::allocate(size_t alignment,
		PermanentGenerationSpec* perm_gen_spec, size_t* _total_reserved,
	...
	size_t s = perm_gen_spec->misc_data_size()
			+ perm_gen_spec->misc_code_size();
	...
	if (UseSharedSpaces) {
		...
		if (UseCompressedOops) {
			heap_address = Universe::preferred_heap_base(total_reserved,
	...
	*_total_reserved = total_reserved;
	*_n_covered_regions = n_covered_regions;
	*heap_rs = ReservedHeapSpace(total_reserved, alignment, UseLargePages,
			heap_address);
...
void GenCollectedHeap::post_initialize() {
	...
	policy->initialize_size_policy(def_new_gen->eden()->capacity(),
			old_gen->capacity(), def_new_gen->from()->capacity());
...
HeapWord* GenCollectedHeap::mem_allocate(size_t size, bool is_large_noref,
		bool is_tlab, bool* gc_overhead_limit_was_exceeded) {
	return collector_policy()->mem_allocate_work(size, is_tlab,
...
void GenCollectedHeap::do_collection(bool full, bool clear_all_soft_refs,
	...
	assert(my_thread->is_VM_thread() ||
			...
			"incorrect thread type capability"); assert(Heap_lock->is_locked(),
			"the requesting thread should have the Heap_lock");
	guarantee(!is_gc_active(), "collection is not reentrant"); assert(max_level < n_gens(), "sanity check");
	...
	if (PrintHeapAtGC) {
		...
		if (Verbose) {
			gclog_or_tty->print_cr("GC Cause: %s",
...

	{
		...
		for (int i = starting_level; i <= max_level; i++) {
			if (_gens[i]->should_collect(full, size, is_tlab)) {
				...
				TraceTime t1(_gens[i]->short_name(), PrintGCDetails, false,
						gclog_or_tty);
...
void GenCollectedHeap::gen_process_strong_roots(int level,
	bool younger_gens_as_roots, bool activate_scope, bool collecting_perm_gen,
	SharedHeap::ScanningOption so, OopsInGenClosure* not_older_gens,
	bool do_code_roots, OopsInGenClosure* older_gens) {
...
if (!do_code_roots) {
	...
	bool do_code_marking = (activate_scope
			|| nmethod::oops_do_marking_is_active());
	CodeBlobToOopClosure code_roots(not_older_gens, /*do_marking=*/
			do_code_marking);
	SharedHeap::process_strong_roots(activate_scope, collecting_perm_gen, so,
...
void GenCollectedHeap::gen_process_weak_roots(OopClosure* root_closure,
	CodeBlobClosure* code_roots, OopClosure* non_root_closure) {
...
assert(Thread::current()->is_VM_thread(), "Precondition#1"); assert(Heap_lock->is_locked(), "Precondition#2");
...
do_collection(true /* full */, clear_all_soft_refs /* clear_all_soft_refs */,
		0 /* size */, false /* is_tlab */, local_max_level /* max_level */);
// Hack XXX FIX ME !!!
...
#ifndef ASSERT
guarantee(
		VerifyBeforeGC || VerifyDuringGC || VerifyBeforeExit || PrintAssembly
...
assert(is_in_reserved(addr), "block_is_obj of address outside of heap"); assert(block_start(addr) == addr, "addr must be a block start");
for (int i = 0; i < _n_gens; i++) {
	if (_gens[i]->is_in_reserved(addr)) {
...
HeapWord* result = mem_allocate(size /* size */, false /* is_large_noref */,
...
assert(_gch != NULL, "Uninitialized access to GenCollectedHeap::heap()"); assert(_gch->kind() == CollectedHeap::GenCollectedHeap, "not a generational heap");
return _gch;
}
...
void GenCollectedHeap::print() const {
print_on(tty);
}
...
GenGCPrologueClosure(bool full) :
_full(full) {
}
;
};
...
GenGCEpilogueClosure(bool full) :
_full(full) {
}
;
};
...
oop GenCollectedHeap::handle_failed_promotion(Generation* gen, oop obj,
size_t obj_size) {
...
public:
GenTimeOfLastGCClosure(jlong now) :
_time(now), _now(now) {
}
