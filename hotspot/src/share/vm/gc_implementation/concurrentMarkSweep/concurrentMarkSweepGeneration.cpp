  ...
  _debug_collection_type(Concurrent_collection_type)
{
  ...
  TRACE_HEAP_INIT(cout << "Initializing the cms space within the cms generation " << endl;)
  TRACE_HEAP_INIT(cout << "Size = " << ((uintptr_t)(end)-(uintptr_t)(bottom)) << endl;)
...
void CMSCollector::do_compaction_work(bool clear_all_soft_refs) {
  ...
  TraceTime t("CMS:Mark-Sweep-Compaction", PrintGCDetails && Verbose, true, gclog_or_tty);
...
void CMSCollector::checkpointRootsInitialWork(bool asynch) {
 ...
 if(_partitionMetaData->getTotalGreyObjectsChunkLevel() != 0){
  ...
  MarkRefsAndUpdateChunkTableClosure notOlder(_span, &_markBitMap, _collectorChunkList, this);
  GenCollectedHeap* gch = GenCollectedHeap::heap();
...
#if OCMS_LOG
	  	  printf("In do_scan_and_mark_OCMS, PageIndex = %d, GreyObjectCount = %d, span is empty()\n",
	  			  scanChunk->getPageIndex(), scanChunk->greyObjectCount());
#endif
	  }
	}
...
MarkRefsAndUpdateChunkTableClosure::MarkRefsAndUpdateChunkTableClosure(
		MemRegion span, CMSBitMap* bitMap,  ChunkList* chunkList, CMSCollector* collector):
				_span(span),
				_bitMap(bitMap),
...
Par_MarkFromGreyRootsClosure::Par_MarkFromGreyRootsClosure(
	...
	_partitionMetaData = _collector->getPartitionMetaData();
	_grey_mark_bit_map = collector->greyBitMap();
}
...
void Par_MarkFromGreyRootsClosure::scan_oops_in_oop(HeapWord* ptr){
	oop obj = oop(ptr);
	if(_grey_mark_bit_map->isMarked(ptr)){ // is already scanned
		return;
	}
//	_partitionMetaData->incrementBytesPage(obj->size()*8, _pageIndex);
	...
	do_yield_check();
	_grey_mark_bit_map->par_mark(ptr);
//	do_throttle_check();
}
