...
void TemplateTable::interceptObject(Address object) {
  ...
  __ movptr(r10, r11);
  __ addptr(r11, base);		  // adding the offset to get the address of the location within memory for the
...
void TemplateTable::_new() {
  ...
  if (UseTLAB || Universe::heap()->supports_inline_contig_alloc()) {
    ...
    __ movptr(Address(rax, oopDesc::counter_offset_in_bytes ()), 0); // #Change, Initializing the counter for new objects
    __ xorl(rcx, rcx); // use zero reg to clear memory (shorter code)
